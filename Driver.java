...
public abstract class Driver {
	...
	static Profile person = null;
	static Scanner input = new Scanner(System.in);
	static Scanner line = new Scanner(System.in);
	private static HashMap<Integer, Profile> list;
	
	public static void manageNetwork(HashMap<Integer, Profile> inbuiltdata){
		list = new HashMap<Integer, Profile>(inbuiltdata);
		...
		do{
			...
			case 2:
				...
				person = profileCheck(list, search);
				if(person != null){
					displayProfile();}
				else{
	...
	public static Profile addProfile(HashMap<Integer, Profile> list){
		...
		String status = null;
		String opt = line.nextLine().toLowerCase();
		switch (opt){
		case "n":
			status = null;
			break;
		...
		if(age<16 && age>2){
			...
			boolean check3 = dependentCheck(list, new String[]{dep1.getName(),dep2.getName()});
			if (dep1 == null || dep2 == null || check3 == false){
				System.out.println("Dependents check Failed !");
			}
			else{
	...
	public static void addFriend(Profile profile, Profile friend){
		if(profile instanceof Child && profile.getAge() - friend.getAge() < 4 && friend instanceof Child){
			...
			System.out.println(friend.getName()+" and "+profile.getName()+" both are not Children or Age difference is greater than 3");
		}
	...
	public static void connectionCheck(HashMap<Integer, Profile> list, String n1, String n2){
